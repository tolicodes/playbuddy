...
import { Community } from '../hooks/CommonContext';
...
const Menu = <T extends { id: string; name: string; code: string; }>({
    ...
    const scrollToLocation = useCallback(() => {
        ...
        logEvent('menu_item_selected', { item: selectedItem?.name });
...
const Dropdown = <T extends { id: string; name: string; code: string }>({
    ...
    const handleSelect = (item: T | null) => {
        onSelectItem(item);
        setIsOpen(false);
        logEvent('defaults_menu_item_selected', { item: item?.name });
    };
...
export const CommunityDropdown: React.FC<CommunityDropdownProps> = ({
    ...
    <Dropdown
        ...
        onSelectItem={(item) => {
            ...
            logEvent('defaults_menu_community_item_selected', { item: item?.name });
        }}
...
export const LocationAreaDropdown: React.FC<LocationAreaDropdownProps> = ({
    ...
    <Dropdown
        ...
        onSelectItem={(item) => {
            onSelectLocationArea(item);
            logEvent('defaults_menu_location_item_selected', { item: item?.name });
        }}
