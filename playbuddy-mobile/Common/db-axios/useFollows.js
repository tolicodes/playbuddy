...
export function useFetchFollows(authUserId) {
    return useQuery({
        queryKey: ['follows', authUserId],
        enabled: !!authUserId,
        queryFn: async () => {
            const res = await axios.get(`${API_BASE_URL}/follows`);
            return res.data;
        },
    });
}
export function useFollow(authUserId) {
...
export function useUnfollow(authUserId) {
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: async (payload) => {
            if (!authUserId) {
