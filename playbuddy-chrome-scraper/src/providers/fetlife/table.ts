...
export const buildStatusTable = (events: EventResult[], skipped?: SkippedEntry[], tableRows?: TableRow[]) => {
    ...
    const rowHtml = rows.map(row => {
        ...
        return `<tr><td>${date}</td><td>${organizer}</td><td>${link}</td><td>${type}</td><td>${statusLabel(row.status)}</td><td>${reason}</td></tr>`;
    });
    const body = rowHtml.length ? rowHtml.join('') : '<tr><td colspan="6">No events yet</td></tr>';
    return `<table border="1" cellspacing="0" cellpadding="6" style="color:#fff;"><thead><tr><th>Date</th><th>Organizer</th><th>Event</th><th>Type</th><th>Status</th><th>Reason</th></tr></thead><tbody>${body}</tbody></table>`;
};
