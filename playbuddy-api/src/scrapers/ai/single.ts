...
export async function extractEventAIOnly(
    ...
    const pagePlatform = classifyPlatform(url);
    ...
    const originalId = deriveOriginalId(url, pagePlatform);
    ...
    const orgOriginalId = deriveOrganizerOriginalId(orgUrl, pagePlatform);
    const resolvedTicketUrl = core?.ticket_url ?? url;
    const resolvedEventUrl = core?.event_url ?? url;
    const ticketingPlatform = classifyPlatform(resolvedTicketUrl || resolvedEventUrl || url);
    const sourceFields = resolveSourceFields({
        eventDefaults,
        sourceUrl: url,
        ticketingPlatform: ticketingPlatform as NormalizedEventInput['source_ticketing_platform'],
        originPlatform: 'website-ai-discovery',
    });
