...
const FLYER_NAME = 'business_card_v1';
...
if (is_business_card && (!frontPngBytes || !backPngBytes)) {
...
async function tileBusinessCards() {
    ...
    for (let p = 0; p < pagesNeeded; p++) {
        const page = doc.addPage([PG_W, PG_H]);
        // Each page places up to 10 front cards
        const batchCount = Math.min(10, totalCards - p * 10);
        for (let i = 0; i < batchCount; i++) {
            const { x, y } = positions[i];
            page.drawImage(frontEmbedded, {
                x,
                y,
                width: SAFE_W,
                height: SAFE_H,
        ...
        console.log(`  📄 Added front side page ${p + 1} with ${batchCount} card(s).`);
    }

    // 6) TILE BACK PAGES (with QR), using filled_pdfs
    for (let p = 0; p < pagesNeeded; p++) {
        const page = doc.addPage([PG_W, PG_H]);
        ...
        console.log(`  📄 Added back side page ${p + 1} with ${batch.length} card(s).`);
    }
    ...
    fs.createReadStream(INPUT_CSV_PATH)
        ...
        .on('data', async (row) => {
            ...
            try {
                ...
                const flyerVersion = String(row['Flyer version'] || '').trim();
                ...
                await processRow(idRaw, flyerVersion, url);
            } catch (err) {
