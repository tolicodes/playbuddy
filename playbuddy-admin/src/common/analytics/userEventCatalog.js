...
const S = {
    entityToAccess: { key: "entity_to_access", type: "string" },
    tabName: { key: "tab_name", type: "string" },
    tab: { key: "tab", type: "string" },
    communityId: { key: "community_id", type: "string" },
    deepLinkSource: {
        key: "source",
        type: "string",
        labelMap: {
    ...
    screenName: { key: "screen_name", type: "string" },
    menuItem: { key: "menu_item", type: "string" },
    mediaEntity: { key: "entity_type", type: "string" },
    tagEntity: { key: "entity_type", type: "string" },
    munchTab: { key: "tab", type: "string" },
};
...
const META_MAP = new Map(USER_EVENT_CATALOG.map((entry) => [entry.event, entry]));
...
export const getUserEventMeta = (eventName) => {
    return META_MAP.get(eventName) ?? null;
};
export const getUserEventDisplayName = (eventName) => {
    return getUserEventMeta(eventName)?.name ?? eventName;
};
export const getUserEventCategory = (eventName) => {
    return getUserEventMeta(eventName)?.category ?? "Other";
};
export const getUserEventStateLabel = (eventName, props) => {
    ...
    if (!str)
        return "Unknown";
    if (state.labelMap && state.labelMap[str])
        return state.labelMap[str];
    return str;
};
